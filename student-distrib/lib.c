...
#define CUR_REG_LOW 0x0F
#define CUR_REG_HIGH 0x0E
...
/*
* void putc_with_scrolling(uint8_t c);
*   Inputs: uint_8* c = character to print
*   Return Value: void
*	Function: Output a character to the console. Scrolls screen_x
*             if at end of video memory.
*/

void
putc_with_scrolling(uint8_t c)
{
    if(c == '\n' || c == '\r') {
...
void update_cursor(int y, int x)
{
   unsigned short position=(y*NUM_COLS) + x;
    ...
    outb(CUR_REG_LOW, CRTC_ADDR);
    outb((unsigned char)(position&BYTE_MASK), CRTC_DATA);
    // cursor HIGH port to vga INDEX register
    outb(CUR_REG_HIGH, CRTC_ADDR);
    outb((unsigned char )((position>>BYTE)&BYTE_MASK), CRTC_DATA);
}

/*
* void scroll_video_mem()
*   Inputs: none
*   Return Value: void
*	Function: Scrolls video memory up one line.
*/
void
scroll_video_mem()
{
...
*   Inputs: x - Address to store screen_x
*           y - Address to store screen_y
*   Return Value: 0 on success, -1 on failure
*	Function: Stores screen_x and screen_y in x and y, repectively.
*/
int 
get_screen_xy(int* x, int* y)
{
	if((x==NULL)||(y==NULL))
		return -1;
